...
<div class="container">
<div class="container" style="background:#C9C4E6;border-radius: 20px;" >
  <br>
  <br>
  <div class="row">
    <div class="col">
      ...
      <div class="row">
       ...
       <div class="col">
        ...
        <a class="btn btn-primary btn-lg active" role="button" aria-pressed="true" href="/l/{{$post->id}}" ><ThumbsupIcon size={16} /><svg class="bi bi-hand-thumbs-up" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        @else
        <a class="btn btn-primary btn-lg active" role="button" aria-pressed="true" href="/nl/{{$post->reactions()->where([['post_id', $post->id],['user_id', Auth::user()->id],])->get()->first()->id}}" ><svg class="bi bi-hand-thumbs-down" width="1em" height="1em" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="container" style="background:#C9C4E6;border-radius: 20px;">
      <tr> 
        <div class="row">
          <div class="col">
            <td><h3 class="text-left"><p>{{$comment->user->name}} : {{$comment->des}}</h3></td>
            </div>
            @auth
            <div class="col">
              @if(Auth::user()->id ==$comment->user_id )

              <td><a class="btn btn-primary" href="/cd/{{$comment->id}}" > delete</a><a class="btn btn-primary" href="/ec/{{$comment->id}}" > edit</a></td>
              @endif
            </div>
          </div>
          <td>
            @foreach ($comment->where('parent_id',$comment->id)->get() as $commentr) 
            <div class="container" style="background: #D5D1E3;border-radius: 20px;" >
              <tr>       
                <td><p class="text-left">{{$commentr->user->name}} replied: {{$commentr->des}}</p></td>
                @auth
                @if(Auth::user()->id ==$commentr->user_id )

                <td><a class="btn btn-primary" href="/cd/{{$commentr->id}}" > delete</a><a class="btn btn-primary" href="/ec/{{$commentr->id}}" > edit</a></td>
